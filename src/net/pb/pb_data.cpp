...
namespace jl
{
    PbRequest::PbRequest() noexcept
        : msg_id_len_(),
          msg_id_(""),
          service_full_name_len_(0),
          service_full_name_(""),
          pb_data_len_(0),
          pb_data_("")
    {
    ...
    PbRequest::PbRequest(const std::string &msg_id, const std::string &service_full_name, const std::string &pb_data) noexcept
        : msg_id_len_(msg_id.size()),
          msg_id_(msg_id),
          service_full_name_len_(service_full_name.size()),
          service_full_name_(service_full_name),
          pb_data_len_(pb_data.size()),
          pb_data_(pb_data)
    {
    }
    ...
    PbResponse::PbResponse() noexcept
        : msg_id_len_(0),
          msg_id_(""),
          pb_data_len_(0),
          pb_data_(""),
          error_code_(static_cast<int32_t>(DataErrorCode::kNoError))
    {
    }

    PbResponse::PbResponse(const std::string &msg_id, const std::string &pd_data, DataErrorCode error_code) noexcept : msg_id_(msg_id),
                                                                                                                       msg_id_len_(msg_id.size()),
                                                                                                                       pb_data_(pd_data),
                                                                                                                       pb_data_len_(pd_data.size()),
